...
fn write_types_files(profile: &FitProfile, fname: &PathBuf) -> Result<(), std::io::Error> {
...
fn write_messages_file(profile: &FitProfile, fname: &PathBuf) -> Result<(), std::io::Error> {
    let mut out = File::create(fname)?;
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_types_files(&profile, &types_fname)?;
    let messages_fname = dest_dir.join("messages.rs");
    write_messages_file(&profile, &messages_fname)?;
